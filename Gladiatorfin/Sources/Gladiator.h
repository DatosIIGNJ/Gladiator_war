...
ALLEGRO_SAMPLE *Camina          = NULL;
ALLEGRO_SAMPLE *Muerte          = NULL;
...
void VictoryS(){
    al_play_sample(Victory,1.0,0.0,1.0,ALLEGRO_PLAYMODE_ONCE,NULL);
}

int CalFit(int Stats[]){
    int Fitnes = (Stats[0]+Stats[1]+Stats[2]+Stats[3]+Stats[4]+Stats[5])/6;
    return  Fitnes;
}
void Genera1(Gladiator Glad[],int tamanho, int Stats[],Data Matriz[FILA][COLUM]){
    ...
    for (int i=0; i<tamanho;i++){
        ...
        Glad[i].Fitnes = CalFit(Stats);
        Glad[i].bordeX = Glad[i].bordeY = 40;
        Glad[i].MGM = Stats[0];
        Glad[i].MGP = Stats[1];
        Glad[i].MGCC = Stats[2];
        Glad[i].RGM = Stats[3];
...
void ActualizaGlads1(Gladiator Glad[],int tamanho, Data Matriz[FILA][COLUM]){
    al_play_sample(Camina,1.0,0.0,1.0,ALLEGRO_PLAYMODE_ONCE,NULL);
    for(int i=0;i<tamanho;i++) {
