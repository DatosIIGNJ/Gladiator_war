...
void Evalua2(int Fila, int Columna, Data Matriz[FILA][COLUM]) {
    for(int i=0;i<8;i++)
        Dir2[i] = false;
    if(Matriz[Fila][Columna-1].used && Matriz[Fila][Columna-1].tipo==1)
        Dir2[0] = true;
...
void ActualizaBalasE2(Proyectil BalaE[],Enemigo Enemy[]){
    for(int i=0;i< 3;i++) {
        Evalua2(Enemy[i].Fila,Enemy[i].Columna,Matriz2);
        if (BalaE[i].distancia >50){
            BalaE[i].x = BalaE[i].IX;
            BalaE[i].y = BalaE[i].IY;
            BalaE[i].distancia = 0;
...
int BalaChocaE(Proyectil BalaE[],Gladiator Glad[]){
    for(int i=0;i< 3;i++) {
        if(Colision(BalaE[i].x,BalaE[i].y,BalaE[i].bordeX,BalaE[i].bordeY,Glad[0].x,Glad[0].y,Glad[0].bordeX,Glad[0].bordeY)){
            BalaE[i].x = BalaE[i].IX;
            BalaE[i].y = BalaE[i].IY;
            Glad[0].RT -= 40;
            break;
...
void DibujaBalasE(Proyectil BalaE[], int Tamanho){
    for(int i=0;i< Tamanho;i++) {
        al_draw_filled_circle(BalaE[i].x+30,BalaE[i].y+20,4,al_map_rgb(184,7,7));
    }
}
