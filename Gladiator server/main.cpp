...
int main() {
    ...
    Camina = al_load_sample("Sonidos/Camina.wav");
    Muerte = al_load_sample("Sonidos/Muerte.wav");
    Victory = al_load_sample("Sonidos/Victory.wav");
    Dalek = al_load_font("Fonts/dalek.ttf",24,0);
    Roman = al_load_font("Fonts/Roman.ttf",18,2);
    ...
    FPSS = al_create_timer(1.0/60);

    al_register_event_source(Evento,al_get_display_event_source(Vent));
    al_register_event_source(Evento,al_get_timer_event_source(FPSS));
    ...
    while(!Salir){
        if((Gladiar1[0].Fila == 2 && Gladiar1[0].Columna==9) && (Gladiar2[0].Fila == 2 && Gladiar2[0].Columna==9)){

            Gladiar1[0].RT = CalculaRT(Gladiar1[0].RGM,Gladiar1[0].RGP,Gladiar1[0].RGCC);
            Gladiar2[0].RT = CalculaRT(Gladiar2[0].RGM,Gladiar2[0].RGP,Gladiar2[0].RGCC);
            Gladiar1[0].Fitnes = CalFit(Stats1);
            Gladiar2[0].Fitnes = CalFit(Stats2);
            if(++Espera >= 120) {
                al_destroy_display(Vent);
                PeleaV(Gladiar1, Gladiar2);
                Salir = true;
                break;
            }
        }
        if ((Gladiar1[0].Fila == 2 && Gladiar1[0].Columna==9) && Gladiar1[0].activo){
            VictoryS();
        }
        if ((Gladiar2[0].Fila == 2 && Gladiar2[0].Columna==9) && Gladiar2[0].activo){
            VictoryS();
        }
        if(true){
            ...
            DibujaGlad(Gladiar1,NumGlads,Matriz1,curFrame,FrameWs,FrameHs);
            DibujaGlad(Gladiar2,NumGlads,Matriz2,curFrame,FrameWs,FrameHs);
            al_flip_display();
        }
    ...
    al_destroy_timer(FPSS);
    al_destroy_sample(Camina);
